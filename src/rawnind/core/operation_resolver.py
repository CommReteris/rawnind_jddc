...
class OperationResolver:
    ...
    def resolve_pipeline(self, pipeline_spec: str) -> List[Tuple[str, Dict[str, Any]]]:
    ...
    def suggest_operations(
        self, context: Optional[str] = None, input_type: Optional[TensorType] = None
    ) -> List[str]:
        ...
        if context:
            ...
            for name, spec in self.registry._specs.items():
                if any(
                    word in spec.description.lower() for word in context_lower.split()
                ):
