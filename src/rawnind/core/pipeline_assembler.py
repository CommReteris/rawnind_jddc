...
from .operation_spec import TensorType
...
class OperationPipeline:
    ...
    def __call__(
        ...
        for i, operation in enumerate(self.operations):
            try:
                current_data, current_metadata = operation(
...
class MetadataDependencyResolver:
    ...
    def resolve_dependencies(
        self, operations: List[str], initial_metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for op_name in operations:
            ...
            for field in spec.metadata_fields_required:
                if field not in available:
                    ...
                    generator = self._find_metadata_generator(
                        field, operations[: operations.index(op_name)]
                    )
                    if generator:
    ...
    def _find_metadata_generator(
        self, field: str, prior_operations: List[str]
    ) -> Optional[str]:
    ...
    def suggest_metadata_operations(self, missing_fields: List[str]) -> List[str]:
