...
def create_training_config_from_yaml(yaml_path: str, **overrides) -> TrainingConfig:
    ...
    def safe_float(value, default):
        """Safely convert value to float, handling strings like '5e-5'."""
        if value is None:
            return default
        try:
            return float(value)
    ...
    def safe_int(value, default):
        """Safely convert value to int, handling strings that represent numbers."""
        if value is None:
            return default
        try:
            return int(float(value))  # Convert to float first to handle '5.0'
        except (ValueError, TypeError):
            return default
