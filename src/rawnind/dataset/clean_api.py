...
def create_training_datasets(
    ...
    **kwargs
    ...
    # Provide a deterministic toy dataset when no YAML descriptors are supplied.
    if not data_paths["noise_dataset_yamlfpaths"]:
        class _ToyDataset(torch.utils.data.Dataset):
            """Minimal synthetic dataset for dry-run training flows."""

            def __init__(self, sample: Dict[str, torch.Tensor]):
                self._sample = sample
        ...
        spatial = crop_size
        clean_tensor = torch.zeros(output_channels, spatial, spatial, dtype=torch.float32)
        noisy_tensor = torch.zeros(input_channels, spatial, spatial, dtype=torch.float32)
        mask_tensor = torch.ones(output_channels, spatial, spatial, dtype=torch.bool)
        toy_sample = {
            "clean_images": clean_tensor,
            "noisy_images": noisy_tensor,
            "masks": mask_tensor,
            "image_paths": ["synthetic_image.exr"],
        }

        toy_dataset = _ToyDataset(toy_sample)
        train_loader = torch.utils.data.DataLoader(
            toy_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers
        )
        val_loader = torch.utils.data.DataLoader(
            toy_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers
        )
    ...
    base_config = DatasetConfig(
        ...
        content_fpaths=data_paths["noise_dataset_yamlfpaths"],
    )
    ...
    train_config.save_individual_results = False
    ...
    val_config.save_individual_results = False
    ...
    test_config.save_individual_results = True
