...
class Encoder(PipelineOperation):
    ...
    def initialize(self):
        ...
        self.encoder_net = nn.Sequential(
            nn.Conv2d(
                3 if self.spec.input_types[0].value == "rgb" else 4,
                32,
                kernel_size=3,
                padding=1,
            ),
            ...
            nn.Conv2d(
                128,
                self.config.get("latent_dim", 256),
                kernel_size=3,
                stride=2,
                padding=1,
            ),
    ...
    def to(self, device: str):
        ...
        Note: When using accelerate, device placement is handled automatically.
        This method is kept for compatibility with non-accelerate usage.
        """
        super().to(device)
        if self.encoder_net is not None:
            # Only move manually if not using accelerate
...
class Decoder(PipelineOperation):
    ...
    def initialize(self):
        ...
        self.decoder_net = nn.Sequential(
            nn.ConvTranspose2d(
                latent_dim, 128, kernel_size=3, stride=2, padding=1, output_padding=1
            ),
            nn.ReLU(),
            nn.ConvTranspose2d(
