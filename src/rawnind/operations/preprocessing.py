...
class RawLoader(PipelineOperation):
    ...
    def forward(
        self, data: torch.Tensor, metadata: Dict[str, Any]
...
class ColorTransform(PipelineOperation):
    ...
    def forward(
        ...
        if len(data.shape) == 3 and data.shape[0] == 4:
            # Convert RGGB to RGB
            rgb = torch.zeros(
                3, data.shape[1] * 2, data.shape[2] * 2, device=data.device
            )
            ...
            # R channel
            rgb[0, 0::2, 1::2] = (
                rgb[0, 0::2, 0::2] + torch.roll(rgb[0, 0::2, 0::2], -1, dims=1)
            ) / 2
            rgb[0, 1::2, :] = (
                rgb[0, 0::2, :] + torch.roll(rgb[0, 0::2, :], -1, dims=0)
            ) / 2

            # G channel (already has most pixels)
            rgb[1, 0::2, 0::2] = (
                rgb[1, 0::2, 1::2] + torch.roll(rgb[1, 0::2, 1::2], 1, dims=1)
            ) / 2
            rgb[1, 1::2, 1::2] = (
                rgb[1, 1::2, 0::2] + torch.roll(rgb[1, 1::2, 0::2], -1, dims=1)
            ) / 2

            # B channel
            rgb[2, 1::2, 0::2] = (
                rgb[2, 1::2, 1::2] + torch.roll(rgb[2, 1::2, 1::2], 1, dims=1)
            ) / 2
            rgb[2, 0::2, :] = (
                rgb[2, 1::2, :] + torch.roll(rgb[2, 1::2, :], 1, dims=0)
            ) / 2
